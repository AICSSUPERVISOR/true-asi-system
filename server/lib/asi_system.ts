...
// Export asiSystem object for test compatibility
export const asiSystem = {
  getPrinciples: () => ASI_PRINCIPLES,
  canSelfImprove: () => true,
  isSafetyEnabled: () => true,
  getSelfModel: () => asiOrchestrator.getSelfModel(),
  getStatistics: () => asiOrchestrator.getStatistics(),
  processTask: (task: AGITask) => asiOrchestrator.processTask(task),
  recursiveSelfImprove: (capability: string, iterations?: number) => 
    asiOrchestrator.recursiveSelfImprove(capability, iterations),
  metaCognition: (topic: string) => asiOrchestrator.metaCognition(topic),
  setGoal: (goal: string) => asiOrchestrator.setGoal(goal)
};
