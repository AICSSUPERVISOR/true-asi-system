...
// Export agiSystem object for test compatibility
export const agiSystem = {
  getCapabilities: () => agiOrchestrator.getAllCapabilities(),
  getAverageCapability: () => {
    const caps = agiOrchestrator.getAllCapabilities();
    return caps.reduce((sum, c) => sum + c.level, 0) / caps.length;
  },
  getSupportedTaskTypes: () => SUPPORTED_TASK_TYPES,
  getIntegrations: () => INTEGRATIONS,
  processTask: (task: AGITask) => agiOrchestrator.processTask(task),
  getStatistics: () => agiOrchestrator.getStatistics()
};
